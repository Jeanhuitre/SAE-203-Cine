<?php
    ...
    try {
        ...
        $res = $dtb->prepare($sql);
        $res->bindParam(':nom', $nom_echappe); /* on utilise bindPram en plus de prepare afin de traîter les données comme des données et non des requêtes */
        $res->bindParam(':visa', $visa);
        $res->bindParam(':duree', $duree);
        $res->bindParam(':rea', $rea);
        $res->bindParam(':genre', $genre);
        $res->bindParam(':synopsis', $synopsis_echappe);
        $res->execute();
